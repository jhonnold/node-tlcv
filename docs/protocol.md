...
- **Message-id restarts**: a fresh session restarts the server's id counter, so `UdpTransport.onMessage` reads a backwards jump into the low end of the range (below `ID_RESTART_WINDOW`) as a restart to follow, and any other backwards step as UDP reordering to discard. This covers restarts the client initiates by re-logging in *and* ones the server initiates on its own; without it a re-login succeeds and every datagram of the new session is dropped as out-of-order.
