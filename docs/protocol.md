...
- **Session loss is silent on the wire**: TLCS can drop a client's login while continuing to answer `PING` with `PONG`, so the socket looks healthy while no broadcast data arrives at all. It sometimes — but not reliably — announces this with `MSG: You are no longer connected to the server.  Please re-connect.`; during the 2026-09-12 incident only 2 of 13 dead broadcasts got that notice. Two things respond to it: `GameService.onServerMessage()` re-logs-in immediately on the notice, and `Broadcast`'s ping interval re-logs-in after `env.dataTimeoutMs` (default 10 min) with no non-keepalive message. The liveness clock ignores `PONG`, `MSG`, and the `LOGON`/`FEATURE`/`level` handshake replies — see `isBroadcastData` in `src/broadcast.ts` — because all of those arrive whether or not the broadcast is alive, and a re-login that is acknowledged but restores no data would otherwise look like recovery. Re-login attempts are rate-limited by a separate timestamp rather than by resetting that clock, so `ccrl_broadcast_seconds_since_data` keeps climbing through failed attempts and stays alertable. Every re-login also clears `UdpTransport.lastMessage`, since a new session restarts the server's id counter and the stale high-water mark would otherwise reject the whole stream as out-of-order.