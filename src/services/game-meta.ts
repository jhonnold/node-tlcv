...
import { siteSlug as toSiteSlug, gameFilenameSlug } from '../util/index.js';
import { pgnPath, writeArchiveFile } from './pgn-storage.js';
...
export async function getMetaFile(siteSlug: string, gameNumber: number): Promise<StoredGameMeta | null> {
  ...
  try {
    const data = await fs.readFile(pgnPath(siteSlug, filename), 'utf-8');
...
export async function saveGameMeta(game: ChessGame, port: number, gameNumber: number, result: string): Promise<void> {
  ...
  const filename = `${gameFilenameSlug(gameNumber, white.name, black.name)}.meta.json`;

  if (await writeArchiveFile(slug, filename, JSON.stringify(meta), port)) {
    metaCache.addFile(slug, gameNumber, filename);
  }
}
